...
import { createNoteController } from '../features/notes/create-note-controller';
...
import { createWarmupQueue, type WarmupStatus } from '../vault-state';
...
export const AppRuntime = () => {
  ...
  const noteController = createNoteController({
    ...
    setRecentNotes,
    getSurface: () => surface,
    getSurfaceElement: () => surfaceMount,
    setPendingZoomRestore: (value) => {
      pendingZoomRestore = value;
    },
    setPendingPositionRestore: (value) => {
      pendingPositionRestore = value;
    },
    clearOutline: () => {
      setOutlineEntries([]);
      setOutlineSelectedIndex(0);
    },
    loadVaultState: () => loadVaultStateOrDefault(),
    showError: (message) => surface?.showError([{ severity: 'error', message }])
  });
  const clearPrefetchTimer = noteController.clearPrefetch;
  const savePositionNow = noteController.savePosition;
  const flushPosition = noteController.flushPosition;
  ...
  const refreshVault = async (vsParam?: VaultStateFileV1): Promise<void> => {
    const generation = noteController.beginNavigation();
    try {
      const snapshot = await window.folea.vault.snapshot();
      if (!noteController.isCurrent(generation)) return;
      ...
      if (targetRelPath && noteController.isCurrent(generation)) {
        await openNoteWithState(targetRelPath, generation, vs);
      } else if (!targetRelPath && noteController.isCurrent(generation)) {
        surface?.clear();
      }
